...
## 1.4.1 출시 후보 범위
- 앱 이름 `급식레벨업`, Bundle ID `com.h19h29.naymnaymlevelup`, 버전 `1.4.1`, 빌드 `41`
...
- 오늘 탭 초입의 `오늘 다 잘먹었어요` 버튼으로 메뉴 상세에 들어가지 않고 오늘 급식 전체를 한 번에 기록. 알레르기 주의 메뉴는 `알레르기로 피했어요`로, 나머지는 `다 먹었어요`로 기록되며 확인 단계에서 주의 메뉴 개수를 안내
...
## 1.4 새 기능
- 오늘 탭 초입에서 `오늘 다 잘먹었어요`로 급식 전체를 한 번에 기록할 수 있어요. 확인 단계에서 메뉴 수와 알레르기 주의 메뉴 수를 보여주고, 주의 메뉴는 안전 상태로만 기록해요.
...
## 출시 전 체크
- `EXPECTED_MARKETING_VERSION=1.4.1 EXPECTED_BUILD_NUMBER=41 RELEASE_UPLOAD_REQUIRED=0 bash scripts/verify-release-readiness.sh`로 로컬 Debug/Release 산출물, plist, 버전/빌드, 메타데이터, 아이콘, 스크린샷, 공개 URL 상태 확인
