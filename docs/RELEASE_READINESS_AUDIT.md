...
코드, 테스트, 시뮬레이터, Release archive, App Store 제출 자료는 1.0 후보 상태까지 준비됐다. TestFlight용 signed build 1.0 (14) IPA는 생성 및 CLI 업로드까지 완료됐다. 외부 테스트 공개는 App Store Connect에서 build 14 처리 완료 확인, 내부/외부 그룹 연결, 외부 테스트 심사 제출이 남아 있다.
...
| 부모 다자녀 연결 | 코드 완료, 운영 설정 확인 필요 | CloudKit `ParentLink` 초대 코드, `childLinkId` 기반 아이별 기록 분리, 아이별 주간 변화 요약, 기록 공유/사진 공유/도전 기록 공유 토글 분리, build 14 export에 Production CloudKit entitlement 포함 |
...
- `git diff --check`: 통과
- `scripts/verify-release-readiness.sh`: 앱 버전/빌드/Bundle ID, CloudKit entitlement, 권한 문구, 추적/위치 권한 부재, 외부 광고/분석/로그인/결제 SDK 부재, build 14 IPA/업로드 로그 증거, App Store 아이콘/스크린샷, 공개 URL 검증 통과
...
- build 14 export summary: buildNumber `14`, versionNumber `1.0`, `beta-reports-active = true`, Cloud Managed Apple Distribution 서명 확인
...
1. App Store Connect에서 build 14 처리 완료 여부 확인
2. build 14를 내부/외부 테스트 그룹에 연결
3. 외부 테스트 그룹 공개 링크에 build 14가 연결됐는지 확인
4. 외부 테스트 심사 제출
