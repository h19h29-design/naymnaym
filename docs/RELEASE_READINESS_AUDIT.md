...
코드, 테스트, 시뮬레이터, signed Release archive, App Store 제출 자료는 1.0 후보 상태까지 준비됐다. TestFlight용 signed build 1.0 (7) IPA는 생성 및 업로드까지 완료됐다. 외부 테스트 공개는 App Store Connect에서 build 7 처리 완료 확인, 외부 그룹 연결, 심사 제출이 남아 있다.
...
| 부모 다자녀 연결 | 코드 완료, 운영 설정 확인 필요 | CloudKit `ParentLink` 초대 코드, `childLinkId` 기반 아이별 기록 분리, build 7에 Production CloudKit entitlement 포함 |
| 부모 공유 사진 | 코드 완료, 운영 설정 확인 필요 | 공유 선택 사진만 `SharedMealPhoto` + CKAsset 생성, build 7에 Production CloudKit entitlement 포함 |
...
| Privacy Manifest | 완료 | `NaymNaymLevelUp/PrivacyInfo.xcprivacy`와 build 7 IPA에 UserDefaults 사유, 선택 부모 공유용 수집 데이터 타입, 추적 없음 선언 포함 |
...
- TestFlight signed IPA: `build/TestFlightExportBuild7Signed/NaymNaymLevelUp.ipa`
- TestFlight build: `1.0 (7)`
...
- build 7 IPA 내부 `Info.plist`: `plutil -lint` 통과
- build 7 IPA 내부 `PrivacyInfo.xcprivacy`와 수집 데이터 타입 포함 확인
- build 7 IPA 내부 `ITSAppUsesNonExemptEncryption = false` 확인
- build 7 IPA 내부 `beta-reports-active = true`, CloudKit Production entitlement 확인
- 최신 XCTest 결과: `build/DerivedData/Logs/Test/Test-NaymNaymLevelUp-2026.06.20_17-44-34-+0900.xcresult`
- 시뮬레이터 스냅샷: 실제 학교 등촌고등학교 상태에서 2026-06-20 토요일 `급식 데이터 없음` 안내 표시, `build/verification/simulator-build7-20260620-175036.png`
- 2026-06-20 추가 갱신: 부모 공유 데이터 흐름에 맞춰 `PrivacyInfo.xcprivacy` 수집 데이터 타입을 보강했고, 해당 매니페스트를 build 7 IPA에 포함해 업로드 완료

## 남은 외부 작업

아래 항목은 로컬 코드로 완료할 수 없고 Apple Developer/App Store Connect 계정에서 확인 또는 처리해야 한다.

1. App Store Connect에서 build 7 처리 완료 여부 확인
2. build 7을 내부/외부 테스트 그룹에 연결
3. 외부 테스트 그룹 공개 링크에 build 7이 연결됐는지 확인
4. 외부 테스트 심사 제출
