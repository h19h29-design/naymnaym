...
코드, 테스트, 시뮬레이터, Release archive, App Store 제출 자료는 1.0 후보 상태까지 준비됐다. TestFlight용 signed build 1.0 (9) IPA는 생성 및 업로드까지 완료됐다. 외부 테스트 공개는 App Store Connect에서 build 9 처리 완료 확인, 외부 그룹 연결, 심사 제출이 남아 있다.
...
| 부모 다자녀 연결 | 코드 완료, 운영 설정 확인 필요 | CloudKit `ParentLink` 초대 코드, `childLinkId` 기반 아이별 기록 분리, 아이별 주간 변화 요약, build 9에 Production CloudKit entitlement 포함 |
| 부모 공유 사진 | 코드 완료, 운영 설정 확인 필요 | 공유 선택 사진만 `SharedMealPhoto` + CKAsset 생성, build 9에 Production CloudKit entitlement 포함 |
...
| Privacy Manifest | 완료 | `NaymNaymLevelUp/PrivacyInfo.xcprivacy`와 build 9 IPA에 UserDefaults 사유, 선택 부모 공유용 수집 데이터 타입, 추적 없음 선언 포함 |
...
- TestFlight signed IPA: `build/TestFlightExportBuild9Signed/NaymNaymLevelUp.ipa`
...
- build 9 IPA 내부 `Info.plist`: `plutil -lint` 통과
- build 9 IPA 내부 `PrivacyInfo.xcprivacy`와 수집 데이터 타입 포함 확인
- build 9 IPA 내부 `ITSAppUsesNonExemptEncryption = false` 확인
- build 9 IPA 내부 `beta-reports-active = true`, CloudKit Production entitlement 확인
- 최신 XCTest 결과: `/Users/mac-mini/Library/Developer/XcodeBuildMCP/workspaces/workspace-f281014df961/result-bundles/test_sim_2026-06-20T09-21-47-732Z_pid97415_571e36a9.xcresult`
- 시뮬레이터 스냅샷: build 9 Debug 실행 화면 캡처, `build/verification/simulator-build9-20260620-182203.jpg`
- 2026-06-20 추가 갱신: 부모 모드에서 아이별 주간 한 입 도전 변화 요약을 표시하도록 보강했고, CloudKit 쿼리의 `createdAt` 서버 정렬 의존성을 제거한 build 9 signed IPA를 App Store Connect에 업로드 완료

## 남은 외부 작업

아래 항목은 로컬 코드로 완료할 수 없고 Apple Developer/App Store Connect 계정에서 확인 또는 처리해야 한다.

1. App Store Connect에서 build 9 처리 완료 여부 확인
2. build 9를 내부/외부 테스트 그룹에 연결
3. 외부 테스트 그룹 공개 링크에 build 9가 연결됐는지 확인
4. 외부 테스트 심사 제출
