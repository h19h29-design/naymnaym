...
코드, 테스트, 시뮬레이터, signed Release archive, App Store 제출 자료는 1.0 후보 상태까지 준비됐다. TestFlight용 signed build 1.0 (6) IPA는 생성 및 업로드까지 완료됐다. 외부 테스트 공개는 App Store Connect에서 build 6 처리 완료 확인, 외부 그룹 연결, 심사 제출이 남아 있다.
...
- build 6 IPA 내부 `Info.plist`: `plutil -lint` 통과
...
1. App Store Connect에서 build 6 처리 완료 여부 확인
2. build 6을 내부/외부 테스트 그룹에 연결
3. 외부 테스트 그룹 공개 링크에 build 6이 연결됐는지 확인
4. 외부 테스트 심사 제출
5. CloudKit Dashboard public database schema 배포 상태 확인
6. CloudKit query index 설정 확인
   - `ParentLink.inviteCode`
   - `SharedMealRecord.childLinkId`
   - `SharedChallengeRecord.childLinkId`
   - `SharedMealPhoto.childLinkId`
7. CloudKit public database 권한 확인
