...
- `FileNutrientImpactSidecar` stores under an Application Support subdirectory, hashes a canonical revision string with existing CryptoKit, rejects traversal/secret/quantity/medical text, writes temp → atomic rename → read-back, never overwrites a prior revision, and returns nil for malformed/schema/rule/fingerprint mismatch. Include a no-op implementation for callers that record without an impact snapshot.
...
- [ ] Step 3: Implement the sidecar with `schemaVersion == 1`, the existing `NutrientImpactSnapshot` copy/disclaimer rules, deterministic canonical fingerprint, safe filename, atomic install/read-back, exact active revision matching, and orphan-tolerant reads. Do not add a Core Data attribute or call any migration/backfill path.
