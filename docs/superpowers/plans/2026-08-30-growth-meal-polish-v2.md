...
- `FileNutrientImpactSidecar` stores under an Application Support subdirectory, hashes a canonical revision string with existing CryptoKit, accepts only `NutrientImpactCopyCatalog` canonical copy for each status and normalized nutrient-ID list, writes temp → atomic rename → read-back, never overwrites a prior revision, and returns nil for malformed/schema/rule/fingerprint mismatch. `SameMealAlternativeSelector` produces a typed `SameMealAlternativeSelection` only from one `RebuildMealDay`, carrying a day fingerprint, current menu, and canonical target nutrient IDs. The selector excludes the current/allergy-overlapping items, keeps only structured nutrient matches, and deterministically prioritizes shared nutrient coverage before the original menu order, capped at two unique labels. `NutrientImpactSnapshotFactory` accepts that typed selection (never raw alternative strings), checks its date/menu/nutrient provenance against the requested snapshot, supplies the canonical copy, and omits the alternative suggestion when the selection is empty. Include a no-op implementation for callers that record without an impact snapshot.
