...
export function SettingsPage() {
  ...
  return <main className="page with-tabs"><header className="page-header"><div><p className="eyebrow">급식레벨업</p><h1>설정</h1></div></header>
    <section className="settings-card"><h2>내 정보</h2><dl><div><dt>별명</dt><dd>{state.profile!.nickname}</dd></div><div><dt>학교</dt><dd>{state.profile!.school.name}</dd></div><div><dt>알레르기</dt><dd>{state.profile!.allergyCodes.length ? `${state.profile!.allergyCodes.length}개 선택` : '선택 안 함'}</dd></div></dl><button onClick={() => navigate('/onboarding')}>학교·알레르기 수정</button></section>
    <section className="settings-card"><h2>안내</h2><button onClick={() => void openExternal(EXTERNAL_URLS.privacy)}>개인정보 처리방침</button><button onClick={() => void openExternal(EXTERNAL_URLS.support)}>문의 및 지원</button><p className="fine-print">별명, 학교, 알레르기와 식사 기록은 이 기기의 토스 저장소에 보관돼요.</p></section>
