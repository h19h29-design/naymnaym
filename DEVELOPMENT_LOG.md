...
- unsigned archive 기반 remote export는 signed app entitlements를 살리지 못했고, signed archive는 올바른 `.xcent` 생성 후 로컬 `codesign`이 keychain/certificate 접근 대기 상태에서 멈췄다.
...
- 로컬 signing keychain 접근 허용 후 build 15 signed archive/export를 생성했고, embedded profile과 signed app CloudKit entitlement 검증을 통과했다.
- `UPLOAD=1 scripts/release-testflight-build.sh 15`로 build 15를 TestFlight에 업로드했고, `scripts/verify-release-readiness.sh` 전체 게이트가 통과했다.
- 제출 전 문서를 build 15 처리 완료 확인, 내부/외부 테스트 그룹 연결, 외부 테스트 심사 제출 대기 상태로 갱신했다.
