...
function isTimeoutError(value: unknown): boolean {
  return isObject(value) && value.name === "TimeoutError";
}
...
export function createHandler(deps: HandlerDeps) {
  return async (request: Request): Promise<Response> => {
    ...
    let neisResultCode: string | undefined;
    ...
    try {
      ...
      if (unwrapped === undefined) {
        status = 403;
        safeReason = "client_token_mismatch";
        return error(
          origin,
          status,
          "FORBIDDEN_ORIGIN",
      ...
      for (let attempt = 0; attempt < MAX_UPSTREAM_ATTEMPTS; attempt++) {
        ...
        try {
          const candidate = await deps.fetch(
          ...
          safeReason = isTimeoutError(cause) ||
              timeoutSignal.aborted && isTimeoutError(timeoutSignal.reason)
            ? "upstream_timeout"
            : "upstream_http_error";
      ...
      if (upstream === undefined) {
        safeReason = "upstream_http_error";
        status = 502;
        return error(
          origin,
          status,
      ...
      if (upstreamBytes === null) {
        status = 502;
        safeReason = "response_parse_error";
        return error(
          origin,
          status,
          "UPSTREAM_ERROR",
