...
Deno.test("never retries upstream 429 and does not leak failure bodies", async () => {
  const sentinel = "private-upstream-body";
  let rateLimitCalls = 0;
  const rateLimited = await createHandler(
    deps(async () => {
      rateLimitCalls++;
      return new Response(sentinel, { status: 429 });
    }),
  )(request({ action: "searchSchools", payload: { keyword: "가람" } }));
...
Deno.test("retries one transient 5xx, then returns the successful response", async () => {
  let calls = 0;
  const response = await createHandler(deps(async () => {
    calls++;
...
Deno.test("uses an 8000ms timeout for each of at most two attempts", async () => {
  ...
  const response = await createHandler({
    ...deps(async (_input, init) => {
      calls++;
      const receivedSignal = init?.signal;
  ...
  assertEquals(calls, 2);
  assertEquals(timeoutValues, [8000, 8000]);
  assertEquals(response.status, 502);
  assertStringIncludes(text, "UPSTREAM_ERROR");
  assertEquals(text.includes(sentinel), false);
...
Deno.test("logs only safe diagnostics, never secrets, payloads, or URLs", async () => {
  const logs: string[] = [];
  const sentinel = "가람-private-user-payload";
  const response = await createHandler(deps(async () => {
    return new Response(JSON.stringify({
      RESULT: { CODE: "INFO-300", MESSAGE: `failed ${API_KEY} ${sentinel}` },
    }));
  ...
  assertEquals(entry.safeReason, "neis_rate_limited");
  assertEquals(entry.neisResultCode, "INFO-300");
