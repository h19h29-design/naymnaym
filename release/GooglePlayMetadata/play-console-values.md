...
- 데이터가 제3자와 공유됨: 제한적으로 예
  ...
  - AI 식단 해설에 동의하면 익명 메뉴별 대표 영양소와 확인된 식단 전체 영양량이 Supabase 및 OpenCode Go에서 처리됨
- 수집 데이터:
  - Health and fitness: 알레르기 선택값, 식사 기록, 먹은 정도
  - User-generated content: 한 입 도전 기록, 어려운 이유, 먹은 정도 기록
  - App interactions: AI 하루 1회 및 남용 방지를 위한 사용 일자와 시도·성공 상태
  - Device or other IDs: AI 한도 적용용 임의 설치 식별값을 복원 불가능한 해시로 서버에 저장
...
급식레벨업은 NEIS 공공데이터 기반 무료 급식 식습관 코칭 앱입니다. 회원가입 없이 `아이로 시작`을 선택하고 별명과 학교를 등록하면 사용할 수 있습니다. 오늘 급식 화면의 AI 식단 해설은 전송 항목에 동의한 뒤 하루 한 번 생성되며 AI가 만든 참고 안내임을 표시합니다. 이름, 학교, 메뉴명, 날짜, 먹은 기록과 등록 알레르기는 AI 제공자에게 보내지 않습니다. 실제 학교 API 실패나 급식 없는 날에는 샘플 식단으로 대체하지 않습니다. 알레르기 번호가 있는 메뉴는 한 입 도전 버튼을 잠그고 학교 안내와 보호자 판단이 우선임을 안내합니다. 부모 공유는 초대 코드로 연결한 뒤 먹은 정도, 한 입 도전 기록, 알레르기 주의만 사용합니다. Android 앱은 현재 알림, 위치, 연락처, 카메라 또는 사진 권한을 요청하지 않습니다.
